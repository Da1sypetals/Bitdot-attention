...
def gen_input(b, h, n, d, d_f):
    ...
    f[perm] = 1.0
    f = f.reshape(1, 1, n, d_f).to(device=device)
    ...
    fint = torch.stack(chunks, dim=1).reshape(-1).to(device=device)  # shape (n*#chunks,)
...
# Benchmark PyTorch implementation with memory tracking
torch.cuda.synchronize()
start = time.time()
...
# Record peak memory for PyTorch implementation
pytorch_peak_memory = torch.cuda.max_memory_allocated() / (1024**3)  # Convert to GB
print("PyTorch (ref):", stop - start)
print(f"PyTorch peak memory: {pytorch_peak_memory:.3f} GB")
...
# Reset memory stats for flex_attention
torch.cuda.reset_peak_memory_stats()
# Benchmark flex_attention implementation with memory tracking
start = time.time()
for _ in trange(N_REP):
    out = flex_attention(
        q,
...
torch.cuda.synchronize()
stop = time.time()
flex_peak_memory = torch.cuda.max_memory_allocated() / (1024**3)  # Convert to GB
stop = time.time()

# Record peak memory for flex_attention implementation
print("flex:", stop - start)
print(f"Flex attention peak memory: {flex_peak_memory:.3f} GB")
